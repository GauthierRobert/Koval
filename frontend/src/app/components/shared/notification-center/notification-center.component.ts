...
import {NotificationCenterService, PersistedNotification} from '../../../services/notification-center.service';
...
@Component({
  ...
  template: `
    <div class="notif-center-overlay" (click)="close.emit()">
      <div class="notif-center-panel glass" (click)="$event.stopPropagation()">
        <div class="notif-center-header">
          ...
          <div class="notif-center-actions">
            ...
            <button class="icon-btn" type="button" (click)="openPrefs.emit()" [attr.aria-label]="'NOTIFICATION_CENTER.PREFERENCES' | translate" [title]="'NOTIFICATION_CENTER.PREFERENCES' | translate">
            ...
            <ul class="notif-list">
              @for (n of items; track n.id) {
                <li class="notif-item" [class.unread]="!n.read" (click)="onClick(n)">
                  ...
                  <button class="notif-delete" (click)="onDelete($event, n.id)" [attr.aria-label]="'NOTIFICATION_CENTER.DELETE' | translate">&times;</button>
                </li>
              }
            </ul>
          }
  ...
  styles: [`
    .notif-center-overlay {
      ...
      padding: 70px 16px 16px; z-index: 1000;
    }
    .notif-center-panel {
    ...
    .notif-empty {
      padding: 30px 16px; text-align: center; opacity: .65; font-size: 13px;
    }
    .notif-list {
    ...
    .notif-item.unread { background: rgba(94,177,255,.06); }
    .notif-item.unread .notif-title { font-weight: 600; }
    .notif-content { flex: 1; min-width: 0; }
    .notif-title { font-size: 13px; }
    .notif-body { font-size: 12px; opacity: .8; margin-top: 2px; }
    .notif-time { font-size: 11px; opacity: .55; margin-top: 4px; }
    .notif-delete {
      background: none; border: none; color: inherit; opacity: .5;
      font-size: 18px; cursor: pointer; padding: 0 4px; line-height: 1;
    }
    .notif-delete:hover { opacity: 1; }
  `],
})

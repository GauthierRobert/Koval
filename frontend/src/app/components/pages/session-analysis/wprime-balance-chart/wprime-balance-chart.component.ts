...
import { pickTickInterval } from '../fit-timeseries-chart/fit-timeseries-chart.utils';
...
export class WPrimeBalanceChartComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  private recompute(): void {
    ...
    const cp =
      this.criticalPower && this.criticalPower > 0
        ? this.criticalPower
        : this.ftp && this.ftp > 0
          ? this.ftp
          : null;
    ...
    const wp = this.userWPrimeJ && this.userWPrimeJ > 0 ? this.userWPrimeJ : this.wPrimeJ;
  ...
  private draw(): void {
    ...
    const drawYLabel = (
      ...
      kjText: string,
      word: string,
      anchor: 'middle' | 'above' = 'middle',
    ) => {
      const kjY = anchor === 'above' ? yPx - 14 : yPx - 6;
      const wordY = anchor === 'above' ? yPx - 2 : yPx + 6;
      ctx.fillStyle = 'rgba(255,255,255,0.7)';
      ctx.fillText(`${kjText} kJ`, padL - 6, kjY);
      ctx.fillStyle = 'rgba(255,255,255,0.4)';
      ctx.fillText(word, padL - 6, wordY);
    };
    drawYLabel(yOf(wp), fullKj, this.axisFull);
    drawYLabel(yOf(wp * 0.5), halfKj, this.axisHalf);
    // Bottom row stacked above the axis line so it doesn't collide with the X-axis time labels.
    drawYLabel(yOf(0), emptyKj, this.axisEmpty, 'above');
    ...
    // X axis (time labels) — evenly spaced ticks matching the fit-timeseries axis cadence.
    const totalSec = this.records[n - 1].timestamp - this.records[0].timestamp || n;
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.textBaseline = 'top';
    const tick = pickTickInterval(totalSec);
    // Start tick left-aligned to the Y axis so it doesn't get clipped by the Y labels.
    ctx.textAlign = 'left';
    ctx.fillText('0:00', padL, padT + h + 4);
    ctx.textAlign = 'center';
    for (let s = tick; s < totalSec - tick / 2; s += tick) {
      const x = padL + (s / totalSec) * w;
      ctx.fillText(`${Math.round(s / 60)}m`, x, padT + h + 4);
    }
    // End label right-aligned so it never overflows the canvas.
    ctx.textAlign = 'right';

...
export class PipService {
    ...
    if ('mediaSession' in navigator) {
      ...
      navigator.mediaSession.setActionHandler('play', async () => {
        ...
        try {
          ...
          console.error('PiP Play Error', e);
        }
      });
  ...
  public async updatePlaybackState(isPaused: boolean) {
    if ('mediaSession' in navigator) {
      ...
      try {
        ...
        /* ignore play errors when hidden */
      }
    }
  }
  ...
  updateCanvas(data: {
    ...
    const nextText =
      data.nextStepPower > 0
        ? `NEXT: ${data.nextStepLabel.toUpperCase()} (${data.nextStepPower}W)`
        : `NEXT: ${data.nextStepLabel.toUpperCase()}`;

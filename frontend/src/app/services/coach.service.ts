...
export class CoachService {
  ...
  assignTraining(
    ...
    return this.http.post<ScheduledWorkout[]>(`${this.apiUrl}/assign`, {
      trainingId,
      athleteIds,
      scheduledDate: date,
      notes,
  ...
  addAthleteGroup(athleteId: string, groupName: string): Observable<User> {
    return this.http.post<User>(`${this.apiUrl}/athletes/${athleteId}/groups`, {
      group: groupName,
    });
  }
  ...
  removeAthleteGroup(athleteId: string, groupName: string): Observable<User> {
    return this.http.delete<User>(
      `${this.apiUrl}/athletes/${athleteId}/groups/${encodeURIComponent(groupName)}`,
    );
  }
  ...
  redeemInviteCode(code: string): Observable<InviteRedemptionResult> {
    return this.http.post<InviteRedemptionResult>(`${this.apiUrl}/redeem-invite`, { code });
  }

  getSessionReminders(): Observable<ScheduledWorkout[]> {
    return this.http
      .get<ScheduledWorkout[]>(`${this.apiUrl}/session-reminders`)
      .pipe(catchError(() => of<ScheduledWorkout[]>([])));
  }

  getAthleteSessions(athleteId: string, limit = 50, page = 0): Observable<SessionData[]> {
    return this.http
      .get<SessionData[]>(`${this.apiUrl}/athletes/${athleteId}/sessions`, {
        params: { limit, page },
      })
      .pipe(catchError(() => of<SessionData[]>([])));
  }

  getSessionById(sessionId: string): Observable<SessionData> {
    return this.http.get<SessionData>(`${environment.apiUrl}/api/sessions/${sessionId}`);
  }

...
import { ZoneSystem, Zone, SportType, ZoneReferenceType } from '../../services/zone';
...
export class ZoneManagerComponent implements OnInit {
    ...
    setActive(system: ZoneSystem) {
        if (!system.id) return;

        this.zoneService.setActiveSystem(system.id).subscribe({
            next: () => {
                this.zoneSystems.forEach(s => {
                    if (s.sportType === system.sportType) {
                        s.isActive = (s.id === system.id);
                    }
                });
            ...
            error: err => console.error('Failed to set active system', err)
        });
    }

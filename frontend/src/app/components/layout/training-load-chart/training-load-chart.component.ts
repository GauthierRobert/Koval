...
// Canonical sport→theme-variable mapping, kept in sync with SportIconComponent.
const SPORT_COLOR_VARS: Record<string, string> = {
  CYCLING: '--success-color',
  RUNNING: '--danger-color',
  SWIMMING: '--secondary-color',
  BRICK: '--accent-color',
  GYM: '--text-muted',
};

function getSportColor(styles: CSSStyleDeclaration, sport?: string): string {
  const cssVar = SPORT_COLOR_VARS[sport || ''] || SPORT_COLOR_VARS['CYCLING'];
  return styles.getPropertyValue(cssVar).trim() || '#FF9D00';
}
  ...
  private draw(): void {
    ...
    dailyData.forEach((d, i) => {
      ...
      for (const [sport, entry] of sportEntries) {
        const color = getSportColor(styles, sport);
      ...
      if (d.totalLoad > 0 && totalBarH > 16 && bw > 20) {
        ...
        ctx.fillText(Math.round(d.totalLoad).toString(), x + bw / 2, H - marginB - totalBarH + 2);
      }

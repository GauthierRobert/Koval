...
export class AuthService {
  ...
  handleStravaCallback(code: string): Observable<{ token: string; user: User }> {
    return this.http
      .get<{ token: string; user: User }>(`${this.apiUrl}/strava/callback?code=${code}`)
      .pipe(
        tap((response) => {
  ...
  handleGoogleCallback(code: string): Observable<{ token: string; user: User }> {
    return this.http
      .get<{ token: string; user: User }>(`${this.apiUrl}/google/callback?code=${code}`)
      .pipe(
        tap((response) => {
          localStorage.setItem('token', response.token);
          this.userSubject.next(response.user);
  ...
  devLogin(
    ...
    return this.http
      .post<{ token: string; user: User }>(`${this.apiUrl}/dev/login`, {
        userId,
        displayName,
        role,
      })
  ...
  unlinkStrava(): Observable<User> {
    return this.http
      .delete<User>(`${this.apiUrl}/link/strava`)
      .pipe(tap((user) => this.userSubject.next(user)));
  }

  unlinkGoogle(): Observable<User> {
    return this.http
      .delete<User>(`${this.apiUrl}/link/google`)
      .pipe(tap((user) => this.userSubject.next(user)));
  }

  linkStravaWithCode(code: string): Observable<User> {
    return this.http
      .post<User>(`${this.apiUrl}/link/strava/callback?code=${encodeURIComponent(code)}`, {})
      .pipe(tap((user) => this.userSubject.next(user)));
  }
  ...
  linkGoogleWithCode(code: string, redirectUri?: string): Observable<User> {
    ...
    return this.http.post<User>(url, {}).pipe(tap((user) => this.userSubject.next(user)));
  }

  unlinkGarmin(): Observable<User> {
    return this.http
      .delete<User>(`${this.apiUrl}/link/garmin`)
      .pipe(tap((user) => this.userSubject.next(user)));
  }

  unlinkZwift(): Observable<User> {
    return this.http
      .delete<User>(`${this.apiUrl}/link/zwift`)
      .pipe(tap((user) => this.userSubject.next(user)));
  }

  updateSettings(settings: Partial<User>): Observable<User> {
    const token = localStorage.getItem('token');
    return this.http
      .put<User>(`${this.apiUrl}/settings`, settings, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .pipe(tap((user) => this.userSubject.next(user)));
  }
  ...
  acceptCgu(): Observable<User> {
    return this.http
      .post<User>(`${this.apiUrl}/cgu/accept`, {})
      .pipe(tap((user) => this.userSubject.next(user)));
  }
}

...
/**
 * Local shim of `fit-file-parser`'s output. The library doesn't re-export
 * `ParsedFit` from its entry, so we declare the subset we actually read.
 */
type ParsedFitData = Parameters<Parameters<FitParser['parse']>[1]>[1];
type RawFitRecord = NonNullable<NonNullable<ParsedFitData>['records']>[number];
type RawFitLap = NonNullable<NonNullable<ParsedFitData>['laps']>[number];

/** The library's event types narrow too aggressively — some FIT files emit numeric
 * codes for `event`/`event_type` rather than the string enums in the .d.ts. */
interface RawFitEvent {
  event?: string | number;
  ...
  parseFitFile(buffer: ArrayBuffer): Promise<FitParseResult> {
    return new Promise((resolve, reject) => {
      ...
      parser.parse(buffer, (error, data) => {
        ...
        const records: FitRecord[] = (data.records || []).map((r: RawFitRecord) => ({
        ...
        if (rawEvents.length > 0 && timerEvents.length === 0) {
          console.warn(
            'FIT file has events but no timer events were parsed. Event sample:',
            rawEvents[0],
          );
        }

        const laps: FitLap[] = (data.laps || []).map((l: RawFitLap) => ({

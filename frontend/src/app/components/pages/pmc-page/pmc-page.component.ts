...
export class PmcPageComponent implements OnInit {
  ...
  // Scheduled workout TSS map: date → { sport → TSS }
  private scheduledTssMap$ = new BehaviorSubject<Map<string, Record<string, number>>>(new Map());
  ...
  fullPmcData$: Observable<PmcDataPoint[]> = combineLatest([
    ...
    map(([real, scheduledMap, days]) => {
      // Today is the last real point; its TSS only carries completed sessions.
      // Fold in today's still-pending scheduled load so the bar reflects what's
      // planned — the projection only covers tomorrow onward.
      const merged = this.mergeTodayScheduledTss(real, scheduledMap);
      return [...merged, ...this.metricsService.projectPmcFromSchedule(merged, scheduledMap, days)];
    }),
  );
      ...
      next: (workouts) => {
        ...
        for (const w of workouts) {
          if (w.status === 'PENDING' && w.tss) {
            ...
            const day = map.get(w.scheduledDate) ?? {};
            day[sport] = (day[sport] ?? 0) + w.tss;
            map.set(w.scheduledDate, day);
          }
        }
  ...
  private fetchAndMerge(from: string, to: string, dir: 'left' | 'right'): void {
    ...
    fetch$.subscribe({
      ...
      complete: () => {
        if (dir === 'left') this.fetchingLeft = false;
        else this.fetchingRight = false;
      },
      error: () => {
        if (dir === 'left') this.fetchingLeft = false;
        else this.fetchingRight = false;
  ...
  /**
   * Attaches today's still-pending scheduled TSS to the last real PMC point as a
   * separate `scheduledSportTss` bucket, so the chart can stack it on the completed
   * load in the scheduled (semi-transparent, dashed) style. It's also folded into
   * `dailyTss` so the TSS panel scales to include it. CTL/ATL are left untouched —
   * they represent actual fitness/fatigue, not forecast.
   */
  private mergeTodayScheduledTss(
    real: PmcDataPoint[],
    scheduled: Map<string, Record<string, number>>,
  ): PmcDataPoint[] {
    if (!real.length || scheduled.size === 0) return real;
    let idx = real.length - 1;
    while (idx >= 0 && real[idx].predicted) idx--;
    if (idx < 0) return real;
    const day = real[idx];
    const sched = scheduled.get(day.date);
    if (!sched) return real;

    const scheduledSportTss = { ...sched };
    const pendingTotal = Object.values(scheduledSportTss).reduce((s, v) => s + v, 0);
    const copy = [...real];
    copy[idx] = { ...day, scheduledSportTss, dailyTss: day.dailyTss + pendingTotal };
    return copy;
  }

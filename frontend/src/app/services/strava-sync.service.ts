...
export class StravaSyncService {
  ...
  importHistory(days = 30): Observable<SyncResult> {
    this.syncingSubject.next(true);
    this.lastResultSubject.next(null);

    return this.http.post<SyncResult>(`${this.apiUrl}/import-history?days=${days}`, {}).pipe(
      tap({
        next: (result) => {
   ...
   */
  buildFit(sessionId: string): Observable<void> {
    return this.http
      .post<void>(`${this.apiUrl}/sessions/${sessionId}/build-fit`, {})
      .pipe(tap(() => this.historyService.reload()));
  }
}

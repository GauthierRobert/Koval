  ...
  private getEstimatedSpeed(
    ...
    if (training.sportType === 'RUNNING') {
      ...
      if (zoneSystem) {
        ...
        let referenceSecondsPerKm: number;

        // Map Zone Reference Types to User Fields
        if (refType === 'THRESHOLD_PACE')
          referenceSecondsPerKm = user.functionalThresholdPace || 240; // 4:00/km
        else if (refType === 'PACE_5K')
          referenceSecondsPerKm = user.pace5k || 220; // 3:40/km
        else if (refType === 'PACE_10K')
          referenceSecondsPerKm = user.pace10k || 240; // 4:00/km
        else if (refType === 'PACE_HALF_MARATHON')
          referenceSecondsPerKm = user.paceHalfMarathon || 270; // 4:30/km
        else if (refType === 'PACE_MARATHON')
          referenceSecondsPerKm = user.paceMarathon || 300; // 5:00/km

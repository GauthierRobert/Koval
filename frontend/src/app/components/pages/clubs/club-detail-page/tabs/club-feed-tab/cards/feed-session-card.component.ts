...
@Component({
      ...
      <div class="session-row">
        ...
        <div class="session-meta">
          ...
          @if (participation.joined) {
            ...
            <span class="waiting-pill">{{ 'CLUB_SESSIONS.WAITING_LIST_POSITION' | translate: { position: participation.waitingListPosition } }}</span>
          ...
          <div class="detail-actions">
            @if (!session.cancelled) {
              @if (!participation.joined && !participation.onWaitingList) {
                <button class="btn-primary small" (click)="join.emit(session.id); $event.stopPropagation()">
                  {{ participation.full ? ('CLUB_SESSIONS.BTN_JOIN_WAITING_LIST' | translate) : ('CLUB_FEED.JOIN' | translate) }}
                </button>
              } @else if (participation.joined) {
                <button class="btn-ghost small" (click)="cancel.emit(session.id); $event.stopPropagation()">
  ...
  `,
  styles: `
    .session-card { background: var(--surface-elevated); border: none; border-radius: var(--radius-sm); padding: 10px; cursor: pointer; transition: border-color 0.15s; }
    .session-card--cancelled { opacity: 0.5; }
    .session-card--selected { background: var(--accent-subtle); }

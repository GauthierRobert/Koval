...
The Claude Code plugin distribution lives in a separate repository ([`GauthierRobert/Koval-plugin`](https://github.com/GauthierRobert/Koval-plugin)) and is maintained independently. Users install it via `/plugin install koval-training@koval-plugin`. Root `skills/` here remains the canonical source — sync changes into the plugin repo manually.

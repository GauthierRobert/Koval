...
export function downsample(records: FitRecord[], bucketSec: number): FitRecord[] {
  ...
  for (let i = 1; i <= records.length; i++) {
    if (i < records.length && records[i].timestamp - records[bStart].timestamp < bucketSec)
      continue;
...
const DRIFT_SKIP_FRAC = 0.1; // discard the first 10% of the session as warmup
const DRIFT_SKIP_CAP_SEC = 180; // …but never skip more than 3 min
const DRIFT_BASE_FRAC = 0.1; // baseline window = 10% of session

...
export class CalendarComponent implements OnInit {
  ...
  clubPrefs: ClubCalendarPreferences = {
    hiddenClubIds: [],
    hiddenGroupIds: [],
    hiddenRecurringTemplateIds: [],
    hideSingleSessions: false,
  };
  myClubRoles: MyClubRoleEntry[] = [];
  clubGroupsMap: Map<string, ClubGroup[]> = new Map();
  // Recurring session series available for filtering, keyed by club id.
  recurringSeriesByClub: Map<string, RecurringSeries[]> = new Map();
  ...
  ngOnInit(): void {
    ...
    const rawClubSessions$ = trigger$.pipe(
      switchMap(() =>
        this.calendarService.getClubSessionsForCalendar(this.startDateKey(), this.endDateKey()),
      ),
      shareReplay(1),
    );

    // Feed the prefs dropdown with whichever recurring series / one-off sessions are
    // currently in range, so the filter only ever lists togglable, real options.
    rawClubSessions$.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((sessions) => {
      this.indexFilterOptions(sessions);
      this.cdr.markForCheck();
    });

    const clubSessions$ = rawClubSessions$.pipe(
  ...
  onReschedule(workout: ScheduledWorkout, newDate: string): void {
    this.calendarService
      .rescheduleWorkout(workout.id, newDate)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(() => this.reload$.next());
  }
  ...
  joinClubSession(session: CalendarClubSession): void {
    this.clubSessionService
      .joinSession(session.clubId, session.id)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(() => this.reload$.next());
  }
  ...
  cancelClubSession(session: CalendarClubSession): void {
    this.clubSessionService
      .cancelSession(session.clubId, session.id)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(() => this.reload$.next());
  }
  ...
  recurringSeriesFor(clubId: string): RecurringSeries[] {
    return this.recurringSeriesByClub.get(clubId) ?? [];
  }

  isRecurringSeriesVisible(templateId: string): boolean {
    return !this.clubPrefs.hiddenRecurringTemplateIds.includes(templateId);
  }

  toggleRecurringSeries(templateId: string): void {
    const idx = this.clubPrefs.hiddenRecurringTemplateIds.indexOf(templateId);
    if (idx >= 0) {
      this.clubPrefs.hiddenRecurringTemplateIds.splice(idx, 1);
    } else {
      this.clubPrefs.hiddenRecurringTemplateIds.push(templateId);
    }
    this.savePreferences();
    this.reload$.next();
  }

  isSingleSessionsVisible(): boolean {
    return !this.clubPrefs.hideSingleSessions;
  }

  toggleSingleSessions(): void {
    this.clubPrefs.hideSingleSessions = !this.clubPrefs.hideSingleSessions;
    this.savePreferences();
    this.reload$.next();
  }
  ...
  private filterByPreferences(sessions: CalendarClubSession[]): CalendarClubSession[] {
    return sessions.filter((s) => {
      ...
      if (s.recurringTemplateId) {
        if (this.clubPrefs.hiddenRecurringTemplateIds.includes(s.recurringTemplateId)) return false;
      } else if (this.clubPrefs.hideSingleSessions) {
        return false;
      }
      return true;
    });
  ...
  /** Derive the distinct recurring series (per club) and single-session presence shown in the filter. */
  private indexFilterOptions(sessions: CalendarClubSession[]): void {
    const byClub = new Map<string, RecurringSeries[]>();
    const seen = new Set<string>();
    let hasSingle = false;
    for (const s of sessions) {
      if (s.recurringTemplateId) {
        if (seen.has(s.recurringTemplateId)) continue;
        seen.add(s.recurringTemplateId);
        const list = byClub.get(s.clubId) ?? [];
        list.push({ templateId: s.recurringTemplateId, clubId: s.clubId, title: s.title });
        byClub.set(s.clubId, list);
      } else {
        hasSingle = true;
      }
    }
    this.recurringSeriesByClub = byClub;
    this.hasSingleSessions = hasSingle;
  }
  ...
  private loadPreferences(): void {
    try {
      ...
      if (stored) {
        const parsed = JSON.parse(stored) as Partial<ClubCalendarPreferences>;
        this.clubPrefs = {
          hiddenClubIds: parsed.hiddenClubIds ?? [],
          hiddenGroupIds: parsed.hiddenGroupIds ?? [],
          hiddenRecurringTemplateIds: parsed.hiddenRecurringTemplateIds ?? [],
          hideSingleSessions: parsed.hideSingleSessions ?? false,
        };
      }
    } catch {
      /* use defaults */
  ...
  private endDateKey(): string {
    return this.viewMode === 'week'
      ? this.weekDays[6].key
      : this.monthDays[this.monthDays.length - 1].key;
  }

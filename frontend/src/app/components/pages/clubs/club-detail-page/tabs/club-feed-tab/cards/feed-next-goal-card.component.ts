...
@Component({
  ...
  template: `
    <div class="feed-card feed-card--goal">
      ...
      <div class="goal-info">
        <app-sport-icon [sport]="$any(event.goalSport ?? 'RUNNING')" [size]="16"></app-sport-icon>
  ...
  styles: `
    ...
    .goal-athletes { display: flex; align-items: center; gap: var(--space-sm); margin-top: var(--space-sm); }
    .avatar-stack { display: flex; }
    .avatar-circle { width: 24px; height: 24px; border-radius: 50%; background: var(--surface-elevated); border: 2px solid var(--bg-color); display: flex; align-items: center; justify-content: center; font-size: 9px; font-weight: 600; color: var(--text-muted); margin-left: -6px; overflow: hidden; flex-shrink: 0; }

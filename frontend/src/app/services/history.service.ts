...
/** Wire format the backend uses for SavedSession before client-side normalisation. */
interface RawSavedSession {
  id: string;
  title: string;
...
export class HistoryService {
  ...
  private loadInitialWindow(): void {
    ...
    this.http
      .get<SessionWindowResponse>(`${this.apiUrl}/window`, {
  ...
  saveSession(summary: SessionSummary, fitBuffer?: ArrayBuffer): void {
    ...
    this.http.post<RawSavedSession>(this.apiUrl, payload).subscribe({
      next: (saved) => {
  ...
  private parseSession = (s: RawSavedSession): SavedSession => ({
    title: s.title,
  ...
  private buildParams(filters: SessionFilters, before: string | null, weeks: number): HttpParams {
    ...
    if (filters.durationMinSec != null)
      params = params.set('durationMinSec', String(filters.durationMinSec));
    if (filters.durationMaxSec != null)
      params = params.set('durationMaxSec', String(filters.durationMaxSec));

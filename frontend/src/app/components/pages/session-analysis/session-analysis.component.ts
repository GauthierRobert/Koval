...
import {
  ...
  ZoneDistEntry,
} from './zone-distribution-panel/zone-distribution-panel.component';
...
export class SessionAnalysisComponent implements OnDestroy {
    ...
    switchMap((session) => {
      ...
      return this.metricsService.downloadStoredFit(session.id).pipe(
        ...
        map((result) => {
          ...
          const movingTime =
            result.totalTimerTime > 0 ? result.totalTimerTime : stripped.movingTime;
          return {
            loading: false,
            error: false,
  ...
  zoneDistribution$: Observable<ZoneDistEntry[]> = combineLatest([
    ...
    map(([fit, user, session, selectedId, userSystems]) => {
      if (!fit || fit.loading || fit.error || fit.records.length === 0 || !session?.sportType)
        return [];
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        const dt =
          i + 1 < records.length ? Math.min(records[i + 1].timestamp - record.timestamp, 30) : 1;
        if (dt <= 0) continue;
  ...
  plannedBlocksWithZones$: Observable<
    (BlockSummary & { zoneLabel?: string; zoneColor?: string; actualSpeedKmh?: number })[]
  > = combineLatest([
    this.displayBlocks$,
    this.sessionSubject,
    this.authService.user$,
  ...
  availableZoneChips$: Observable<{ label: string; color: string }[]> = this.zoneDistribution$.pipe(
    map((zones) =>
      zones.filter((z) => z.seconds > 0).map((z) => ({ label: z.label, color: z.color })),
    ),
  );

  filteredZoneBlocks$: Observable<ZoneBlock[]> = combineLatest([
    this.zoneBlocks$,
    this.zoneFilters$,
  ]).pipe(
    map(([blocks, filters]) =>
  ...
  > = combineLatest([this.plannedBlocksWithZones$, this.zoneFilters$]).pipe(
    map(([blocks, filters]) =>
      filters.size === 0
        ? blocks
        : blocks.filter((b) => b.zoneLabel != null && filters.has(b.zoneLabel)),
    ),
  );

...
export class WorkoutExecutionService {
  ...
  private archiveCurrentBlock() {
    ...
    const target =
      block.type === 'RAMP'
        ? Math.round(((((block.intensityStart || 0) + (block.intensityEnd || 0)) / 2) * ftp) / 100)
        : Math.round(((block.intensityTarget || 0) * ftp) / 100);
  ...
  private tick() {
    ...
    const elapsed = state.elapsedTotalSeconds + 1;
  ...
  private subscribeToMetrics() {
    ...
    this.metricsSubscription = this.bluetoothService.metrics$.subscribe((metrics) => {
      ...
      const blockPower = Math.round(
        blockHistory.reduce((acc, m) => acc + m.power, 0) / blockHistory.length,
      );
      const blockCadence = Math.round(
        blockHistory.reduce((acc, m) => acc + m.cadence, 0) / blockHistory.length,
      );
      const blockSpeed = Number(
        (blockHistory.reduce((acc, m) => acc + m.speed, 0) / blockHistory.length).toFixed(1),
      );
      const blockHRSamples = blockHistory.filter((m) => m.heartRate !== undefined);
      const blockHR =
        blockHRSamples.length > 0
          ? Math.round(
              blockHRSamples.reduce((acc, m) => acc + m.heartRate!, 0) / blockHRSamples.length,
            )
          : 0;

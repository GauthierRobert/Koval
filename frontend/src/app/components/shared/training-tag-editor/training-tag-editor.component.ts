 ...
 * - Pressing Enter (or comma) or tapping the confirm icon commits the current
 *   input. Pressing Backspace on an empty input removes the last chip.
 *
...
export class TrainingTagEditorComponent {
  ...
  private refreshSuggestions(): void {
    ...
    this.suggestions = (query ? pool.filter((t) => t.includes(query)) : pool).slice(0, 6);
    this.cdr.markForCheck();
  }
}
